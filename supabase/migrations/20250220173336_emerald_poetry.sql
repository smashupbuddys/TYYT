...
DROP TRIGGER IF EXISTS format_phone_trigger ON customers CASCADE;
DROP FUNCTION IF EXISTS format_phone_before_save() CASCADE;
DROP FUNCTION IF EXISTS format_phone_number(text, text) CASCADE;
...
-- Make email nullable and add phone constraints
...
CREATE OR REPLACE FUNCTION format_phone_before_save()
RETURNS TRIGGER AS $$
BEGIN
  -- Format phone number
  NEW.phone := format_phone_number(NEW.phone, COALESCE(NEW.country_code, 'IN'));
...
UPDATE customers 
SET phone = format_phone_number(phone, COALESCE(country_code, 'IN'));