...
CREATE OR REPLACE FUNCTION validate_workflow_status_v7(status jsonb)
RETURNS boolean AS $$
BEGIN
...
ALTER TABLE video_calls
  ...
  CHECK (validate_workflow_status_v7(workflow_status));

-- Create function to initialize workflow status
CREATE OR REPLACE FUNCTION initialize_workflow_status_v2()
RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER initialize_workflow_status_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION initialize_workflow_status_v2();
...
WHERE workflow_status IS NULL 
   OR NOT validate_workflow_status_v7(workflow_status);