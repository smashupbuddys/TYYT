...
BEGIN
  ...
  IF NOT p_is_staff_update THEN
    ...
    -- Format notice period message
    v_notice_message := format(
      'Video calls must be scheduled at least %s hours in advance for %s customers. Please select a time after %s',
      v_notice_hours,
      v_customer_type,
      to_char(v_now + (v_notice_hours || ' hours')::interval, 'Mon DD, YYYY, HH12:MI AM')
    );

    -- Check notice period with proper error message
    IF p_scheduled_at <= (v_now + (v_notice_hours || ' hours')::interval) THEN
      RAISE EXCEPTION '%', v_notice_message;
    END IF;
  END IF;
